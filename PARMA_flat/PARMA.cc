...
PARMA::PARMA()
{
  ...
  ptCloud = NULL;
  dataSplits = NULL;
  maps = new ParmaConcurrentMap[numberOfSplits];
  useDelimitingBox = true;

  combineTasks = new std::vector<CombineTask*>();

  for (int i = 0; i < numberOfSplits; i++)
    {
      for (int j = i + 1; j < numberOfSplits; j++)
	{
	  combineTasks->push_back(new CombineTask(i, j));
	}
    }

  assert(combineTasks->size() == ((numberOfSplits*(numberOfSplits-1))/2));//debug assert
}
...
void PARMA::performLocalRelabeling(std::vector<idxXYZ>* data, ParmaConcurrentMap* map)
{
  assert(data != NULL);
  assert(map  != NULL);
...
void PARMA::performLocalClustering(std::vector<idxXYZ>* data, ParmaConcurrentMap* map,
				   int offset,
				   bool useDelimitingBox)
{
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices->begin ();
       ...
       ++it)
    {
      ParmaConcurrentEllipsoid* ellipsoid = new ParmaConcurrentEllipsoid(offset + j);
