...
do
	...
	do
	    item=" -f ../../datasets/geolife/geolife_subset.txt -m ${MinPts} -e ${Epsilon} -a 1 -K ${K} -S ${S} -O ${outputDir}"
	    ...
	    command_accuracy="${executable_randindex}./baseline/labeling/geolife_subset.txt_baseline.idx ${outputDir}/labeling/geolife_subset.txt_${method}_K_${K}_S_${S}.idx ${outputDir}/stats/accuracy.txt"
	    $command_accuracy
	done
    done
done
